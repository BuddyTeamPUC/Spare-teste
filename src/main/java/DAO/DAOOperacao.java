...
public class DAOOperacao {
	...
	public boolean inserirOperacao(operacao operacao) {
		...
		try {  
			...
			st.executeUpdate("INSERT INTO Operacao (data,valor,tipo, descricao, id) " + "VALUES ('"+ operacao.getData()+ "', '" + operacao.getValor() + "', '"  + operacao.getTipo() + "', '" + operacao.getDescricao() + "', '" + operacao.getID() + "');");
			st.close();
			status = true;
			...
			ResultSet rs = st.executeQuery("SELECT * FROM Operacao");		
	         if(rs.next()){
	             ...
	             for(int i = 0; rs.next(); i++) {
	                Operacao[i] = new operacao(rs.getDate("data"), rs.getInt("valor"), rs.getString("tipo"), rs.getString("descricao"), rs.getInt("id"));
	             }
	          }
	          st.close();
		} catch (Exception e) {
